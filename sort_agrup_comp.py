...
# ----------------------------
uploaded_compradores = st.file_uploader("Selecione o arquivo exportacao_painel_compradores_s2", type=['xlsx'], key="compradores")
if uploaded_compradores is not None:
    ...
    st.info("Carregue o arquivo exportacao_painel_compradores_s2.")
    st.stop()
...
# ----------------------------
uploaded_controle = st.file_uploader("Selecione o arquivo Controle Processos Publicação SCOMP2 (opcional)", type=['xlsx'], key="controle")
controle_dict = {}
...
autorizados_preg = ["FERNANDA GALVÃO", "GONCIRITA SEIXAS", "RICARDO OLIVEIRA", "NELSON FORESTI", "JOSE LIMA"]
autorizados_preg = [nome.upper() for nome in autorizados_preg]
...
# ----------------------------
# 5. Algoritmo de Distribuição Greedy
# ----------------------------
# allocation_result: dicionário para armazenar os códigos de agrupamento atribuídos para cada comprador.
allocation_result = defaultdict(list)

for g in sorted_groupings:
    candidates = []
    for buyer, info in eligible_buyers.items():
        # Verifica se o comprador tem capacidade: allocated < min(target_qp, 15)
        if info["allocated"] < min(info["target_qp"], 15):
            # Se o agrupamento for PREG, apenas compradores autorizados
            if ("PREG" in g) and (buyer not in autorizados_preg):
                continue
            candidates.append(buyer)
    if not candidates:
        continue
    # Seleciona o candidato com maior shortfall (deficit para atingir 120)
    ...
    eligible_buyers[selected]["shortfall"] = max(0, 120 - (eligible_buyers[selected]["PDT"] + eligible_buyers[selected]["QIC"] + eligible_buyers[selected]["allocated"]))
